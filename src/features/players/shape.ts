 ...
 * the tests run under jest with relative imports, like metadata.ts.
 *
 * ONE copy, two clients. This lives in the app tree and the web imports it
 * through a re-export shim (web/src/players/shape.ts), the way the web
 * referee runs the app's scoring engine rather than a port — so a tag made on
 * the phone and a tag made in the browser store byte-for-byte the same
 * summary, and a profile reads the same on both. Pure TypeScript on purpose:
 * no react-native, no expo, and only `../analysis/types` (the authoritative
 * analysis.json contract) imported, so both bundlers and both jest runs can
 * reach it. A documented cross-feature import (docs/01 rule 2): the contract
 * file is the schema reference every consumer of an analysis reads.
 *
...
export interface Player {
  ...
  notes: string;
  /**
   * Set when the owner has minted a share link: anyone holding
   * /p/<token> can read this profile (and only this one) via the
   * `shared_player` RPC. Null means private. Absent from shared reads.
   */
